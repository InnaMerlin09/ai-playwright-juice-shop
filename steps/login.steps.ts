...
import { navigateToLoginPage, loginWithCredentials } from '../pages/LoginPage';
import { expectHomePageToBeDisplayed } from '../assertions/HomePageAssertions';
...
When('the user logs in with valid credentials', async ({ page }) => {
    await loginWithCredentials(page, env.testUserEmail, env.testUserPassword);
});

Then('the homepage should be displayed', async ({ page }) => {
